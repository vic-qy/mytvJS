/**
 * ysp.js —— 央视频（YangShipin）直播源解析脚本（mytv 版）
 *
 * 由 K9/ysp.js（酷9版）移植：协议逻辑逐字节保留，仅替换 HTTP 层与入口/出口。
 *
 * 频道（仅这 4 个）：
 ...
 *
 * 用法（mytv 频道地址）：
 *   .../js/ysp.js?id=cctv6      // 单频道播放地址
 *   .../js/ysp.js?id=list       // 全部频道 m3u8（默认）
 *
 ...
 *
 * 注意：央视频接口约 1/3 概率拒绝下发播放地址（服务端策略），
 *       yspFetchOne 已内置 3 次重试；id=list 时逐频道自动重试。
 */
...
// 统一 HTTP：mytv.request（headers 为 "k: v" 数组，返回 {body, code, headers, message}）
function yspHttp(url, method, headers, body) {
  var arr = [], k;
  for (k in headers) if (Object.prototype.hasOwnProperty.call(headers, k)) arr.push(k + ': ' + headers[k]);
  try {
    var res = mytv.request(url, (method || 'GET').toUpperCase(), arr, body || '');
    var txt = res ? res.body : '';
    if (typeof txt !== 'string') txt = (txt === undefined || txt === null) ? '' : JSON.stringify(txt);
    return { code: (res && typeof res.code === 'number') ? res.code : 200, text: txt };
...
function yspFetchOne(key) {
  var ch = CHANNELS[key];
  if (!ch) return null;
  var last = null, i, sdkToken, pt, r;
  for (i = 0; i < 3; i++) {
    sdkToken = yspSdkToken();
    if (!sdkToken) return { error: 'SDK token 获取失败' };
    pt = yspPlayerToken(ch.pid);
    if (!pt || !pt.token) return { error: '播放令牌获取失败' };
    r = yspPlayUrl(ch, sdkToken, pt);
    if (r && r.url) return r;
    if (r && r.error && r.error.indexOf('cKey') >= 0) return r; // cKey 本地生成失败，重试无意义
    last = r;
  }
  return last || { error: '获取失败' };
}
...
// ===========================================================================
// 7. 入口（mytv：main(paramstr) 返回 JSON 字符串）
// ===========================================================================
function main(paramstr) {
  ...
  if (id === 'list') {
    var list = yspBuildList();
    if (list && list.m3u8) return JSON.stringify({ m3u8: list.m3u8 });
    return JSON.stringify({ url: '', error: (list && list.error) || '列表获取失败' });
  }

  if (!Object.prototype.hasOwnProperty.call(CHANNELS, id)) {
    return JSON.stringify({ url: '', error: '无效的频道ID: ' + id + '，可用：cctv6, tj, xj, jy1, list' });
  }

  var r = yspFetchOne(id);
  if (!r || !r.url) return JSON.stringify({ url: '', error: (r && r.error) || '获取失败' });
  return JSON.stringify({
    url: r.url,
    headers: 'User-Agent: ' + YSP.ua + '\nReferer: https://www.yangshipin.cn/'
  });
}
